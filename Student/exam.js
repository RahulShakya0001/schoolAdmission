$(document).ready(function () {
    console.log(login.relation_id);
    let user_data = db.student.find((v) => v.id == login.relation_id);
    let student_class = user_data.class;
    let find_class = db.class.find((i) => i.id == Number(student_class));
    console.log(find_class);

    let exam_data = db.exam.filter((v) => Number(v.class) == find_class.id);
    ...
    exam_data.forEach((v, i) => {
        v.question_data.forEach((vv, ii) => {
            let sub_text = db.subject.find((s) => s.id == Number(v.subject));
            html += '<tr>';
            html += '<td>' + vv.id + '</td>';
            html += '<td>' + v.title + '</td>';
            ...
            if (typeof exit == "undefined" && v.status == 1) {
    ...
    $(".result-button").click(function (e) {
        ...
        let result_id = $(this).attr("res_id") * 1;
