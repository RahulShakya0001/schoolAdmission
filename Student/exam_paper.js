$(document).ready(function () {
    ...
    let exam_sub_id = ids.length > 1 ? ids[1] * 1 : null;
    let login_student = db.student.find((v) => v.id == login.relation_id);

    let exam_data = db.exam.reduce((a, c) => {
        let loginclass = db.student.find((x) => x.id == login_student.id);
        let actual_login_cl = db.class.find((v) => v.id == loginclass.class * 1);
        let actual_login_cls = db.exam.find((v) => v.class == actual_login_cl.class);
        if (c.class == actual_login_cls.class) {
            c.assign.forEach((vv) => {
